...
def _add_recon(report: HwatReport, session: HwatSession, *, nmap: bool) -> Optional[str]:
    ...
    report.recon = ReconResult(target=session.target, ip=ip, ok=True)
    if not (nmap and ip):
        return ip

    try:
        recon_sess = recon_mod.init(session.target, ip=ip)
        if recon_sess is None:
            report.recon.ok = False
            report.recon.error = "Could not initialize recon session"
            return ip
        nmap_res = recon_mod.nmap_scan_typed(target=ip, session=recon_sess)
        report.recon.nmap = nmap_res
        try:
            banners = recon_mod.banner_grab(host=ip, ports=list(getattr(nmap_res, "open_tcp", []) or []))
            report.recon.banners = {int(k): v for k, v in (banners or {}).items()}
        except Exception as e:
            logger.exception("Banner grab failed target=%s ip=%s: %s", session.target, ip, e)
    except Exception as e:
        logger.exception("Recon phase failed target=%s ip=%s: %s", session.target, ip, e)
        report.recon.ok = False
...
def _add_dns(
    ...
    *,
    ...
    try:
        ...
        logger.exception("DNS phase failed target=%s: %s", target, e)
...
def _add_web(report: HwatReport, session: HwatSession, *, url: Optional[str]) -> None:
    ...
    try:
        ...
        logger.exception("Web phase failed target=%s url=%s: %s", session.target, base_url, e)
...
def _add_privesc(report: HwatReport) -> None:
    try:
        ...
        report.privesc = PrivescResult(ok=True, raw=raw, score=privesc_mod.risk_score(raw))
    except Exception as e:
        logger.exception("Privesc phase failed: %s", e)
        report.privesc = PrivescResult(ok=False, raw={}, score=None, error=str(e))
...
def _add_secrets(report: HwatReport, *, secrets_paths: Optional[List[str]]) -> None:
    ...
    try:
        ...
        logger.exception("Secrets phase failed paths=%s: %s", secrets_paths, e)
...
def _add_plugins(report: HwatReport, session: HwatSession, *, plugins: Optional[Iterable[str]]) -> None:
    ...
    try:
        ...
        report.plugins = _serialize_plugin_results(results)
    except Exception as e:
        logger.exception("Plugins phase failed plugins=%s: %s", list(plugins), e)
        report.plugins = {"ok": False, "error": str(e)}


def _serialize_plugin_results(results: Dict[str, Any]) -> Dict[str, Any]:
    return {k: (v.to_dict() if hasattr(v, "to_dict") else v.__dict__) for k, v in results.items()}
...
def _add_fingerprint(report: HwatReport, ip: Optional[str]) -> None:
    ...
    for port in ports:
        try:
            ...
            logger.warning("Fingerprint failed ip=%s port=%s error=%s", ip, port, e)
